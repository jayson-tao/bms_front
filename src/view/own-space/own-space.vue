    ...
    <Card>
      ...
      <div>
        ...
        >
          ...
          <FormItem label="系统角色：" prop="role_text">
            <div style="display:inline-block;width:300px;">
              <Input v-model="userForm.role_text"></Input>
            </div>
          </FormItem>

          <!--<FormItem label="用户手机：" prop="telephone">-->
            <!--<div style="display:inline-block;width:204px;">-->
              <!--<Input v-model="userForm.telephone" @on-keydown="hasChangePhone"></Input>-->
            <!--</div>-->
          ...
          <!--<FormItem label="用户手机：">-->
          <!--<Button type="text" size="small" @click="showEditPassword">绑定手机号</Button>-->
          <!--</FormItem>-->
          <!--<div>-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getIdentifyCode() {
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            ...
            axios.request({
              url: 'user/identifyCode/'+this.userForm.telephone,
              method: 'get'
            })
          }
        })
      ...
      ,
      sureRole() {
        var role = this.userForm.role
        switch (role) {
          case 0:
            this.userForm.role_text = "读者";
            break;
          case 1:
            this.userForm.role_text = "管理员";
            break;
          case 2:
            this.userForm.role_text = "超级管理员";
            break;
        }
      }
      ,
      ...
      init() {
        axios.request({
          ...
          this.userForm.name = resp.data.name
          this.userForm.telephone = resp.data.telephone
          this.userForm.age = resp.data.age
          this.userForm.role = resp.data.role
          this.sureRole()
        })
